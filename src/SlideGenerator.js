...
const SlideGenerator = {
   ...
   */
  generatePresentation: function (templateId, folderId, municipio, uf, results) {
    const outputFolder = DriveApp.getFolderById(folderId);
    const templateFile = DriveApp.getFileById(templateId);

    const newFileName = `MSL_Apresentacao_${municipio}_${uf}_${
    ...
    const totalSummaryText = `R$ ${formatTotalSummary(totalSum)}`;
  ...
  createTableOnSlide: function (slide, municipio, uf, results, totalSum) {
    ...
    const titleShape = slide.insertShape(SlidesApp.ShapeType.TEXT_BOX, left, top - 30, width, 30);
    const title = titleShape.getText();
    title.setText(titleText);
    title.getTextStyle().setFontSize(14).setBold(true).setForegroundColor(0, 0, 0);
    ...
    validResults.forEach((result, index) => {
      ...
      [cell0, cell1].forEach((cell) => {
        cell.getText().getTextStyle().setFontSize(10).setBold(false).setForegroundColor(0, 0, 0);
      });
    ...
    [totalCell0, totalCell1].forEach((cell) => {
      cell.getText().getTextStyle().setFontSize(12).setBold(true).setForegroundColor(0, 0, 0);
    });
